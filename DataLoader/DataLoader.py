...
class YahooFinanceDataLoader:
    ...
    def __init__(self, number_of_index, dataset_name, split_point, begin_date=None, end_date=None, load_from_file=False):
        """
        :param dataset_name
        ...
        # if load_from_file=False in main.py
    ...
    def load_data(self):
        ...
        @return:
        """
        data = pd.read_csv(f'{self.DATA_PATH}{self.DATA_FILE}', sep=';')

        data.dropna(inplace=True)
        data.set_index('Date', inplace=True)
        #Tobias: added additional features
        #Tobias: if for 1_index or 2_index version
        if self.number_of_index ==1:
            data.rename(columns={'Close': 'close', 'Open': 'open', 'High': 'high', 'Low': 'low','Close_1': 'close_1', 'Open_1': 'open_1', 'High_1': 'high_1', 'Low_1': 'low_1'}, inplace=True)

        elif self.number_of_index ==2: 
    ...
    #Tobias: added additional features for preprocessing
    def normalize_data(self):
        ...
        if self.number_of_index ==1:
            ...
            self.data['open_norm_1'] = min_max_scaler.fit_transform(self.data.open_1.values.reshape(-1, 1))
            self.data['high_norm_1'] = min_max_scaler.fit_transform(self.data.high_1.values.reshape(-1, 1))
            self.data['low_norm_1'] = min_max_scaler.fit_transform(self.data.low_1.values.reshape(-1, 1))
            self.data['close_norm_1'] = min_max_scaler.fit_transform(self.data.close_1.values.reshape(-1, 1))
            ...
            self.data['open_norm_2'] = min_max_scaler.fit_transform(self.data.open_1.values.reshape(-1, 1))
            self.data['high_norm_2'] = min_max_scaler.fit_transform(self.data.high_1.values.reshape(-1, 1))
            self.data['low_norm_2'] = min_max_scaler.fit_transform(self.data.low_1.values.reshape(-1, 1))
            self.data['close_norm_2'] = min_max_scaler.fit_transform(self.data.close_1.values.reshape(-1, 1))
