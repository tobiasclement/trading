...
class Data:
    def __init__(self, number_of_index, data, action_name, device, gamma, n_step=4, batch_size=50, start_index_reward=0,
                 transaction_cost=0):
        """
    ...
    def get_reward(self, action):
        ...
        #Tobias: removed this reward_index_last as it is buggy, see solution below
        ...
        #Tobias: fixed bug, as index would be go beyond len(self.close_price)
        if self.current_state_index + self.n_step + self.start_index_reward < len(self.close_price):
            reward_index_last = self.current_state_index + self.start_index_reward + self.n_step
        else: reward_index_last = len(self.close_price) - 1
